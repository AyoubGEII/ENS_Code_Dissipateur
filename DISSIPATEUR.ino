...
//------------ Derniere VERSION - H. Sebastien - O. Ayoub (stagiaire IUT) - 13/06/22 -------
...
// ------------------------ CONFIGURATION BROCHES EN ENTREE ET EN SORTIE -----------------------   
   ...
   timer.setInterval(0.5,protection); // la fonction BF_i sera appelee toutes les 500 microsecondes
 }
// --------------------------------------- BOUCLE ------------------------------------------------
...
// -------------------------------- MESURE ET CALCULE DE LA TENSION --------------------------------------
  ...
  // pour 1024 on a 5V donc pour v_result on fait un simple produit en croix puis on realise le produit x100
  // due a la sonde
   
// ------------------------------------------ TESTS DES ETATS  --------------------------------------
  if(v_result >= 250) // PAS (delta) DE 10 ENTRE LES DEUX ETATS POUR 250
    etat_250 = 255; // ENVOIE +15V
  else
  if(v_result <= 240) // PAS (delta) DE 10 ENTRE LES DEUX ETATS POUR 250
    etat_250 = 0; // RESTE A 0
  
  if(v_result >= 200)
    etat_200 = 255; // ENVOIE +15V
  else
  if(v_result <= 190)
    etat_200 = 0; // RESTE A 0

  if(v_result >= 150)
    etat_150 = 255;// ENVOIE +15V
  else
  if(v_result <= 140)
